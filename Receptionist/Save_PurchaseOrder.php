  ...
  $p_code = $_POST['p_code'];  
	...
	$query=mysqli_query($Wooden_AuraConnection,"select * from purchase_order_hold ")or die(mysqli_error($Wooden_AuraConnection));
		while ($row=mysqli_fetch_array($query)) // select all material from Invoice to save into second table with forgin key
		{
			...
			mysqli_query($Wooden_AuraConnection,"INSERT INTO purchase_orderdetails(p_code,material_id,qty,Requested_id) VALUES('$p_code','$material_id','$qty','$Request_id')")or die(mysqli_error($Wooden_AuraConnection));
...
$query=mysqli_query($Wooden_AuraConnection,"select * from purchase_orderdetails  order by p_code DESC LIMIT 1")or die(mysqli_error()); 
...
if ($result == 0) {
 ...
 $rec= mysqli_fetch_assoc($query);
  ...
  $num = substr($lastid, 3);
  $num++;
  $newidNew = "P" . str_pad($num, 5, "0", STR_PAD_LEFT);
}
?>
...
<script type="text/javascript"> 
  ...
  <!--End Auto calcuation -->
                ...
                <div class="form-group col-md-6">
                  ...
                  <input class="form-control" type="text"  name="p_code" value="<?php echo($newidNew)?>" readonly >
                </div>
