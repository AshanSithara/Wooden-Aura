  ...
  // Save to first table
	...
	$query=mysqli_query($Wooden_AuraConnection,"select * from sales_hold ")or die(mysqli_error($Wooden_AuraConnection));
		while ($row=mysqli_fetch_array($query)) // select all item from Invoice to save into second table with forgin key
		{
			...
			mysqli_query($Wooden_AuraConnection,"INSERT INTO Salesdetails(prod_id,qty,price,Requested_id) VALUES('$pid','$qty','$price','$Request_id')")or die(mysqli_error($Wooden_AuraConnection));
		
      	// save into backup table
			mysqli_query($Wooden_AuraConnection,"INSERT INTO Salesbackup(prod_id,qty,price,Requested_id) VALUES('$pid','$qty','$price','$Request_id')")or die(mysqli_error($Wooden_AuraConnection));
